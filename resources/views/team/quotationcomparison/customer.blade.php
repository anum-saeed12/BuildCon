                            ...
                            <table class="table table-hover text-nowrap table-compact">
                                ...
                                @forelse($quotations as $quotation)
                                    <tr style="cursor:pointer" class="no-select" data-toggle="modal"
                                        data-href="#">
                                        <td><a href="{{ route('comparison.edit.team',$quotation->id) }}">{{ $loop->iteration }}</td>
                                        <td><a href="{{ route('comparison.edit.team',$quotation->id) }}">{{ ucfirst($quotation->customer_name) }}</td>
                                        <td><a href="{{ route('comparison.edit.team',$quotation->id) }}">{{ ucfirst($quotation->project_name) }}</td>
                                        <td><a href="{{ route('comparison.edit.team',$quotation->id) }}">{{ ucfirst($quotation->competitor_name) }}</td>
                                        <td><a href="{{ route('comparison.edit.team',$quotation->id) }}">{{ \Carbon\Carbon::parse($quotation->date)->format('d-M-Y')}}</td>
                                        <td><a href="{{ route('comparison.edit.team',$quotation->id) }}">{{ $quotation->total }}</td>
                                        <td><a href="{{ route('comparison.edit.team',$quotation->id) }}">{{ ucfirst($quotation->total_comparison) }}</td>
                                        <td class="text-right p-0">
                                            <a class="bg-primary list-btn" href="{{ route('comparison.edit.team',$quotation->id) }}" title="Edit"><i class="fas fa-tools" aria-hidden="false"></i></a>
                                            <a class="bg-danger list-btn"  href="{{ route('comparison.delete.team',$quotation->id) }}" title="Delete"><i class="fas fa-trash-alt" aria-hidden="false"></i></a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
