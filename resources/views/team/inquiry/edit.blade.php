                    ...
                    <form class="form-horizontal" action="{{ route('inquiry.update.team',$inquiry->id) }}" method="POST">
                        ...
                        <div class="card-body pb-0">
                            ...
                            <div class="row">
                                <div class="col">
                                    <small>
                                        <span>Displaying <b>{{ $inquiry->items->count() }}</b> of <b>{{ $inquiry->total_items }}</b> {{ intval($inquiry->total_items)>1?'items':'item' }}</span>
                                        <span>&dash; <b>{!! intval($inquiry->total_items) - intval($inquiry->items->count()) !!}</b> more {{ (intval($inquiry->total_items) - intval($inquiry->items->count()))>1?'items':'item' }} belonging to different {{ (intval($inquiry->total_items) - intval($inquiry->items->count()))>1?'categories':'category' }}</span>
                                    </small>
                                    <hr class="mt-1"/>
                                </div>
                            </div>
