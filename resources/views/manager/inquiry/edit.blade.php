                        ...
                        <div class="card-body pb-0">
                            ...
                            <div class="additional-products">
                                @foreach($inquiry->items as $inquiry_item)
                                    ...
                                    <div class="row mt-3">
                                        <div class="col-md-3 category-container">
                                            <select name="category_id[]" class="form-control form-control-sm" id="category_id_{{ $loop->iteration }}" data-target="#item_id_{{ $loop->iteration }}" data-href="{{ route('category.fetch.ajax.manager') }}" data-spinner="#item_spinner_{{ $loop->iteration }}" onchange="categorySelect($(this))">
                                                <option selected="selected" value>Select Category</option>
                                                @foreach ($categories as $category)
                                        ...
                                        <div class="col-md-2 brand-container">
                                            <select name="brand_id[]" class="form-control form-control-sm" id="brand_id_{{ $loop->iteration }}">
                                                <option selected="selected" value>Select Brand</option>
                                                @foreach (fetchBrandsForItem($inquiry_items->item_name) as $brand)
                                                    <option value="{{ $brand->id }}"{{ $inquiry_items->brand_id==$brand->id ?' selected':'' }}>{{ ucfirst($brand->brand_name) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col quantity-container">
