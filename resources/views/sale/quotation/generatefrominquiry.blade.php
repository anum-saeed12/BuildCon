            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card card-info">
                        <form class="form-horizontal" action="{{ route('quotation.store.sale',$inquiry->quotation_id) }}" method="POST">
                            @csrf
                            ...
                            <div class="card-body pb-0">
                                <div class="row">
                                    <div class="col-md">
                                        ...
                                        <select name="customer_id" class="form-control form-control-sm" id="customer_id">
                                            ...
                                            @foreach ($customers as $customer)
                                                <option value="{{ $customer->id }}"{{ $inquiry->customer_id==$customer->id ? ' selected':'' }}>{{$customer->customer_id}}{{ ucfirst($customer->customer_name) }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md">
                                        ...
                                        <input type="text" name="currency" class="form-control form-control-sm" id="currency" value="{{ $inquiry->currency }}" placeholder="Example: USD, PKR" required>
                                    </div>
                                </div>
                                <br/>
                                <div class="row">
                                    <div class="col-md-2 category-container">
                                        ...
                                        <select name="category_id[]" class="form-control form-control-sm categories" id="category_id" data-target="#item_id" data-href="{{ route('category.fetch.ajax.sale') }}" data-spinner="#category_spinner" onchange="categorySelect($(this))">
                                    ...
                                    <div class="col-md-2 item-container">
                                        ...
                                        <select name="item_id[]" class="form-control form-control-sm trigger" id="item_id" data-target="#brand_id" data-href="{{ route('item.fetch.ajax.sale') }}" data-spinner="#item_spinner" onchange="itemSelect($(this))">
                                            <option selected="selected" value>Select</option>
                                    ...
                                    <div class="col-md-2 brand-container">
                                        ...
                                        <select name="brand_id[]" class="form-control form-control-sm" id="brand_id"
                                                ...
                                                data-item="#item_id"
                                                data-href="{{ route('item.fetch.ajax.sale') }}"
                                    ...
                                    <div class="col quantity-container">
                                        ...
                                        <input type="text" name="quantity[]" value="{{ $inquiry->items[0]->quantity }}"
                                    ...
                                    <div class="col rate-container">
                                        ...
                                        <input type="text" name="rate[]" value="{{ $inquiry->items[0]->rate }}"
                                    ...
                                    <div class="col discount_rate-container">
                                        <label for="discount_rate"><span id="dc_txt">{{ $inquiry->discount }}</span>% Off</label><br/>
                                        <input type="text" name="discount_rate[]" class="form-control form-control-sm with_out discounted_rate"
                                               ...
                                               onkeydown="calculate($(this))" onkeypress="calculate($(this))"
                                               onkeyup="calculate($(this))" onchange="calculate($(this))"
                                               value="{{ $inquiry->items[0]->discount_rate }}" readonly>
                                    </div>
                                    <div class="col amount-container">
                                        <label for="amount">Sub-Total</label><br/>
                                        <input type="text" name="amount[]" value="{!! floatval($inquiry->items[0]->rate) * intval($inquiry->items[0]->quantity) !!}" class="form-control form-control-sm total n" id="total_amount">
                                    </div>
                                    <div class="col-0">
                                        ...
                                        <div class="row mt-3">
                                            <div class="col-md-2 category-container">
                                                <select name="category_id[]" class="form-control form-control-sm categories" id="category_id" data-target="#item_id" data-href="{{ route('category.fetch.ajax.sale') }}" data-spinner="#category_spinner" onchange="categorySelect($(this))">
                                                    <option selected="selected" value>Select Category</option>
                                                    @foreach ($categories as $category)
                                                        <option value="{{ $category->id }}"{{ $inquiry_item->category_id==$category->id ? ' selected':'' }}>{{ ucfirst($category->category_name) }}</option>
                                                    @endforeach
                                                </select>
                                                <div id="category_spinner"></div>
                                                <div class="text-danger">@error('category_id'){{ $message }}@enderror</div>
                                            ...
                                            <div class="col-md-2 item-container">
                                                {{--<label for="item_id_{{ $loop->iteration - 1 }}">Select Item </label><br/>--}}
                                                <select name="item_id[]" class="form-control form-control-sm trigger" id="item_id_{{ $loop->iteration - 1 }}" data-target="#brand_id_{{ $loop->iteration - 1 }}" data-href="{{ route('item.fetch.ajax.sale') }}" data-spinner="#item_spinner_{{ $loop->iteration - 1 }}" onchange="itemSelect($(this))">
                                                    <option selected="selected" value>Select</option>
                                            ...
                                            <div class="col quantity-container">
                                                ...
                                                <input type="text" name="quantity[]" value="{{ $inquiry_item->quantity }}"
                                            ...
                                            <div class="col unit-container">
                                                ...
                                                <input type="text" name="unit[]" value="{{ $inquiry_item->unit }}" class="form-control form-control-sm" id="unit_{{ $loop->iteration - 1 }}" >
                                            </div>
                                            <div class="col rate-container">
                                                {{--<label for="rate_{{ $loop->iteration - 1 }}">Rate</label><br/>--}}
                                                <input type="text" name="rate[]" value="{{ $inquiry_item->rate }}"
                                            ...
                                            <div class="col discount_rate-container">
                                                {{--<label for="discount_rate_{{ $loop->iteration - 1 }}"><span id="dc_txt_{{ $loop->iteration - 1 }}">{{ $inquiry->discount }}</span>% Off</label><br/>--}}
                                                <input type="text" name="discount_rate[]" class="form-control form-control-sm with_out discounted_rate"
                                                       ...
                                                       onkeydown="calculate($(this))" onkeypress="calculate($(this))"
                                                       onkeyup="calculate($(this))" onchange="calculate($(this))"
                                                       value="{{ $inquiry_item->discount_rate }}" readonly>
                                            </div>
                                            <div class="col amount-container">
                                                {{--<label for="amount_{{ $loop->iteration - 1 }}">Sub-Total</label><br/>--}}
                                                <input type="text" name="amount[]" value="{!! floatval($inquiry_item->rate) * intval($inquiry_item->quantity) !!}" class="form-control form-control-sm total n" id="total_amount_{{ $loop->iteration - 1 }}">
                                            </div>
                                            <div class="col-0">
                                                {{--<label for="unit">&nbsp;</label><br/>--}}
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <textarea class="form-control form-control-sm" name="terms_condition" id="terms_condition" required> {{ $inquiry->terms_condition }}</textarea>
                                    </div>
                                </div>
                                <br/>
                                <div class="row">
        ...
        $(document).ready(function() {
            ...
            $(add_button).click(function(e) {
                ...
                let $itemRow = '<div class="row mt-3">' +
                    '<div class="col-md-2 category-container">' +
                    `<select name="category_id[]" class="form-control form-control-sm categories" id="category_id_${$uid}" data-target="#item_id_${$uid}" data-href="{{ route('category.fetch.ajax.sale') }}" data-spinner="#category_spinner_${$uid}" onchange="categorySelect($(this))">` +
                    '<option selected="selected" value>Select Category</option>' +
                    @foreach ($categories as $category)
                    ...
                    '<div class="col-md-2 item-container">' +
                        ...
                        `<select name="item_id[]" class="form-control form-control-sm" id="item_id_${$uid}" data-target="#brand_id_${$uid}" data-href="{{ route('item.fetch.ajax.sale') }}" data-spinner="#item_spinner_${$uid}" onchange="itemSelect($(this))">` +
                            '<option selected="selected" value>Select</option>' +
                        '</select>' +
